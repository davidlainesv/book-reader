...
export default function ChatbotPanel({
  ...
  return (
    <div className="h-full w-full flex flex-col bg-gradient-to-b from-blue-50/50 to-indigo-50/50 dark:from-blue-950/20 dark:to-indigo-950/20 rounded-lg font-sans">
      {/* Chat Header */}
      <div className="px-4 py-3 border-b border-blue-200/50 dark:border-blue-800/50 bg-white/60 dark:bg-gray-800/60 backdrop-blur-sm rounded-t-lg">
        <h1 className={`text-2xl font-bold text-blue-900 dark:text-blue-100 ${controlClass}`} 
                style={{ fontSize: `${Math.max(fontSize * 1.3, 24)}px` }}>
              🤖 Reflexiona con la IA
            </h1>
        <p className="text-sm text-blue-700 dark:text-blue-300">Reflexiona sobre lo que has aprendido</p>
      </div>
      ...
      <div ref={listRef} className="flex-1 h-0 overflow-auto space-y-3 p-4">
        {messages.map((m, idx) => (
          <div
            ...
            key={idx}
            className={`max-w-[85%] rounded-2xl px-4 py-3 custom-font-size custom-line-height shadow-sm ${
              m.role === "assistant" 
        ...
        {isGeneratingResponse && (
          <div className="max-w-[85%] rounded-2xl px-4 py-3 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 border border-gray-200/50 dark:border-gray-700/50 shadow-sm custom-font-size custom-line-height">
            <div className="flex items-center gap-2">
              <span className="text-sm opacity-70">La IA está pensando</span>
